...
namespace FhirOnAzure.Engine.Core
{
    ...
    public class FhirPropertyIndex
    {
        ...
        /// </summary>
        /// <param name="fhirModel">IFhirModel that can provide mapping from resource names to .Net types</param>
        public FhirPropertyIndex(IFhirModel fhirModel, Assembly fhirAssembly) : this(fhirModel,
            LoadSupportedTypesFromAssembly(fhirAssembly))
        {
        }
        ...
        ///     <see cref="findPropertyInfo(Type, string)" />.
        /// </summary>
        ...
        /// <summary>
        ///     Find info about the properties in <paramref name="fhirType" /> that are of the specified
        ///     <paramref name="propertyType" />.
        /// </summary>
        /// <param name="fhirType">Type of resource that should contain a property with the supplied name.</param>
        ...
        public IEnumerable<FhirPropertyInfo> findPropertyInfos(Type fhirType, Type propertyType,
            bool includeSubclasses = false)
        {
            var propertyPredicate = includeSubclasses
                ? (pi => pi.AllowedTypes.Any(at => at.IsAssignableFrom(propertyType)))
                : new Predicate<FhirPropertyInfo>(pi => pi.AllowedTypes.Contains(propertyType));

            return findFhirTypeInfo(r => r.FhirType == fhirType)
                .findPropertyInfos(propertyPredicate);
        }

        /// <summary>
        ///     Find info about the  properties that adhere to <paramref name="propertyPredicate" />, in the types that adhere to
        ///     <paramref name="typePredicate" />.
        ...
        /// <returns></returns>
        public IEnumerable<FhirPropertyInfo> findPropertyInfos(Predicate<FhirTypeInfo> typePredicate,
            Predicate<FhirPropertyInfo> propertyPredicate)
        {
            return findFhirTypeInfos(typePredicate)?.SelectMany(fti => fti.findPropertyInfos(propertyPredicate));
        }

        /// <summary>
        ///     Find info about the first property that adheres to <paramref name="propertyPredicate" />, in the types that adhere
        ///     to <paramref name="typePredicate" />.
        ///     This is a very generic function. Check whether a more specific function will also meet your needs.
        ///     (Thereby reducing the chance that you specify an incorrect predicate.)
        ///     If you want to get all results, use
        ///     <see cref="findPropertyInfos(Predicate{FhirTypeInfo}, Predicate{FhirPropertyInfo})" />.
        /// </summary>
        /// <param name="typePredicate">predicate that the type(s) must match.</param>
        /// <param name="propertyPredicate">predicate that the properties must match.</param>
        /// <returns></returns>
        public FhirPropertyInfo findPropertyInfo(Predicate<FhirTypeInfo> typePredicate,
            Predicate<FhirPropertyInfo> propertyPredicate)
        {
        ...
        internal FhirPropertyInfo CreateFhirPropertyInfo(PropertyInfo prop)
        {
            ...
            result.TypedNames = result.AllowedTypes
                .Select(at => result.PropertyName +
                              findFhirTypeInfo(fti => fti.FhirType == at)?.TypeName.FirstUpper() ?? at.Name).ToList();
        ...
        private void ExtractReferenceTypes(PropertyInfo prop, FhirPropertyInfo target)
        {
            ...
            if (attReferenceAttribute != null)
            {
                ...
                target.AllowedTypes.AddRange(attReferenceAttribute.Resources
                    .Select(r => _fhirModel.GetTypeForResourceName(r)).Where(at => at != null));
            }
        }
    ...
    public class FhirPropertyInfo
    {
        ...
        /// <summary>
        ///     A path in a searchparameter denotes a specific type, as propertyname + Typename, e.g.
