...
namespace FhirOnAzure.Engine.Search
{
    using System;
    ...
    public class ElementIndexer
    {
        ...
        private List<Expression> ToExpressions(FhirBoolean element)
        {
            ...
            var values = new List<IndexValue>();
            values.Add(new IndexValue("code",
                element.Value.Value ? new StringValue(bool.TrueString) : new StringValue(bool.FalseString)));
        ...
        private List<Expression> ToExpressions(Quantity element)
        {
            ...
            {
                _log.InvalidElement("unknown",
                    string.Format("Quantity: {0} {1} {2}", element.Code, element.Unit, element.Value), ex.Message);
            }
            return null;
        }
