...
namespace FhirOnAzure.Engine.Search.Model
{
    ...
    using System.Linq;
    using FhirOnAzure.Search.Support;
    ...
    public class ActualModifier
    {
        ...
        public ActualModifier(string rawModifier)
        {
            ...
            Modifier = mapping
                .FirstOrDefault(m => m.Key.Equals(rawModifier, StringComparison.InvariantCultureIgnoreCase)).Value;
            ...
            if (parts[0].Equals(missing, StringComparison.InvariantCultureIgnoreCase))
            {
                if (parts.Length > 1)
                    if (parts[1].Equals(MISSINGTRUE, StringComparison.InvariantCultureIgnoreCase))
                        ...
                        throw Error.Argument("rawModifier",
                            "For the :missing modifier, only values '{0}' and '{1}' are allowed", MISSINGTRUE,
                            MISSINGFALSE);
                return true;
            }
            return null;
