    ...
    public static class Validate
    {
        ...
        public static void IsResourceIdEqual(IKey key, Resource resource)
        {
            if (key.ResourceId != resource.Id)
                throw Error.BadRequest("The Id in the request '{0}' is not the same is the Id in the resource '{1}'.",
        ...
        public static void IsSameVersion(IKey orignal, IKey replacement)
        {
            if (orignal.VersionId != replacement.VersionId)
                throw Error.Create(HttpStatusCode.Conflict,
                    "The current resource on this server '{0}' doesn't match the required version '{1}'", orignal,
                    replacement);
        }
